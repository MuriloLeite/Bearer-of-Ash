...
GameManager.attributes.add("enemyPrefab", {
  type: "entity",
  title: "Enemy Prefab (disabled template entity)",
});
...
GameManager.prototype.initialize = function () {
  try {
    var saved =
      pc.platform.browser &&
      window &&
      window.localStorage.getItem("ash:difficulty");
...
GameManager.prototype._updateAltar = function () {
  if (this.altar && this.altar.script && this.altar.script.altar) {
    // Quando não existem tochas na cena (sistema antigo removido),
    // garantimos que o altar funcione com 4 velas como objetivo padrão.
    var totalForAltar = this._totalTorches > 0 ? this._totalTorches : 4;
    this.app.fire("altar:update", this._candlesLit, totalForAltar);
  }
};

GameManager.prototype._onCandleLit = function (candleCount) {
  this._candlesLit = candleCount;

  var multiplier = 1 + candleCount * 0.3;
  this._difficultyParams.enemySpeed =
    this._getDifficultyTable()[this.difficulty].enemySpeed * multiplier;
  this._difficultyParams.spawnInterval =
    this._getDifficultyTable()[this.difficulty].spawnInterval / multiplier;

  this._enemies.forEach(function (enemy) {
    if (enemy && enemy.script && enemy.script.enemyAI) {
      enemy.script.enemyAI.speedChase = this._difficultyParams.enemySpeed;
    }
  }, this);
...
GameManager.prototype._updateHUD = function () {
  // Quando removemos o sistema de tochas do mapa, o total fica 0.
  // Para a mecânica atual (acender 4 velas no altar), exibimos 4 como padrão.
  var totalForHud = this._totalTorches > 0 ? this._totalTorches : 4;
  this.app.fire("hud:update", {
    lit: this._candlesLit,
    total: totalForHud,
...
GameManager.prototype._onVictory = function () {
  ...
  // Mensagem simples de vitória via HUD/DOM
  this.app.fire("ui:hint", "🎉 Vitória! O círculo mágico foi completado.");
...
GameManager.prototype.update = function (dt) {
  ...
  if (this._pendingVictory) {
    ...
    if (this._victoryCheckTime <= 0) {
      ...
      for (var i = 0; i < this._torchEntities.length; i++) {
        var ts =
          this._torchEntities[i].script && this._torchEntities[i].script.torch;
        if (!ts || !ts.isLit()) {
          allLit = false;
          break;
        }
...
GameManager.prototype.resetGame = function () {
  ...
  for (var j = 0; j < this._torchEntities.length; j++) {
    var ts2 =
      this._torchEntities[j].script && this._torchEntities[j].script.torch;
