/*
  ...
  - Player lights (hold E)
  - Enemy extinguishes
  - Notifies GameManager via app events
  - Uses sprite frames: 0 = unlit, 1 = lit (recommended)
*/
...
// Optional for engine-only: textures to use if no Sprite component
...
Torch.prototype.initialize = function () {
  ...
  // Se começar apagada, esconder render mas manter entidade ativa
  if (!this._isLit && this.entity.render) {
...
Torch.prototype._applySprite = function () {
  // Sempre usa a textura acesa (lateral) quando a tocha está visível
  var frameIndex = 1; // litFrameIndex

  if (this.entity.sprite) {
    this.entity.sprite.frame = frameIndex;
  }

  // Render material sempre usa textura acesa
  if (this.entity.render) {
    var torchTextures = window.GAME_TEXTURES?.torch || [];
    var texture = null;

    if (torchTextures.length >= 2) {
      texture = torchTextures[1]; // Sempre usa tocha-lateral (acesa)
    } else if (this.litTexture) {
      texture = this.litTexture.resource || this.litTexture;
    }
    ...
    if (texture) {
      ...
      mat.emissive = new pc.Color(1, 0.9, 0.6); // Sempre brilhante quando visível
...
Torch.prototype.beginIgnite = function (playerEntity) {
  ...
  if (this._igniteBy && this._igniteBy !== playerEntity) return; // already in progress by another
  this._igniteBy = playerEntity;
  // progress is kept if player re-presses quickly (optional). Reset for clarity:
  // this._igniteProgress = 0;
};
...
Torch.prototype.update = function (dt) {
    ...
    if (stillHolding && withinReach) {
      ...
      this.app.fire(
        ...
        "Igniting... " +
          Math.min(
            100,
            Math.floor((this._igniteProgress / this.igniteTime) * 100)
          ) +
          "%"
      );
  ...
  if (this._extinguishBy && this._isLit) {
    // Optional: check still near (usando XY ao invés de XZ)
...
Torch.prototype._setLit = function (lit) {
  ...
  // Mostrar/esconder apenas o render da tocha
  if (this.entity.render) {
    this.entity.render.enabled = lit;
  }
