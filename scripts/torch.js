...
Torch.attributes.add('igniteTime', { type: 'number', default: 0.8, title: 'Ignite Hold Time (s)' }); // Reduzido de 1.2 para 0.8
Torch.attributes.add('extinguishTime', { type: 'number', default: 2.5, title: 'Extinguish Time (s)' }); // Aumentado de 2.0 para 2.5
...
Torch.prototype._applySprite = function () {
    ...
    if (this.entity.render) {
        ...
        if (tex) {
            ...
            // No texture - use color: RED when unlit, BRIGHT YELLOW when lit
            ...
            var color = this._isLit ? new pc.Color(1, 0.9, 0.2) : new pc.Color(0.6, 0.1, 0.1); // Amarelo brilhante / Vermelho escuro
            mat2.diffuse = color;
            mat2.emissive = color;
...
Torch.prototype.update = function (dt) {
    ...
    if (this._igniteBy && !this._isLit) {
        ...
        if (stillHolding && withinReach) {
            ...
            this.app.fire('ui:hint', 'Igniting... ' + Math.min(100, Math.floor(this._igniteProgress / this.igniteTime * 100)) + '%');
