...
function loadTextures(app, urls) {
  return new Promise((resolve) => {
    const assets = urls.map(
      (url, i) => new pc.Asset(`esm_tex_${Date.now()}_${i}`, "texture", { url })
    );
    ...
    function done() {
      loaded++;
      if (loaded === assets.length) resolve(out);
    }
...
async function main() {
  ...
  difficultyText.addComponent("element", {
    type: pc.ELEMENTTYPE_TEXT,
    anchor: [1, 1, 1, 1],
    pivot: [1, 1],
    fontSize: 28,
  ...
  const btnReturnToMain = createButton(
    app,
    pausePanel,
    "Return to Main",
    new pc.Vec3(0, -80, 0),
  ...
  loseLabel.addComponent("element", {
    type: pc.ELEMENTTYPE_TEXT,
    anchor: [0.5, 0.5, 0.5, 0.5],
    pivot: [0.5, 0.5],
    fontSize: 48,
  ...
  // Load assets (unificados) de ./game_assets/**
  const texMapArr = await loadTextures(app, [
    "./game_assets/world/scenario.png",
  ]);
  const texMap = texMapArr[0];
  const heroTex = await loadTextures(app, [
    "./game_assets/player/player_front.png",
    "./game_assets/player/player_back.png",
    "./game_assets/player/player_side.png",
  ]);
  const enemyTex = await loadTextures(app, [
    "./game_assets/enemy/enemy_front.png",
    "./game_assets/enemy/enemy_back.png",
    "./game_assets/enemy/enemy_side.png",
  ]);
  const altarTex = await loadTextures(app, [
    "./game_assets/magic/magic_circle.png",
    "./game_assets/magic/magic_circle_01.png",
    "./game_assets/magic/magic_circle_02.png",
    "./game_assets/magic/magic_circle_03.png",
    "./game_assets/magic/magic_circle_completed.png",
  ]);
  const torchUnlit = null;
  const torchLit = null;
  const uiFontAsset = null; // opcional
  ...
  player.script.create("playerController", {
    attributes: { frameTextures: heroTex },
  });
  ...
  // Força fallback de texturas usado pelo script do altar
  ...
  for (let i = 0; i < torchPositions.length; i++) {
    ...
    t.script.create("torch", {
      attributes: {
        startLit: false,
        unlitTexture: torchUnlit,
        litTexture: torchLit,
      },
    });
  ...
  enemyPrefab.script.create("enemyAI", {
    attributes: { frameTextures: enemyTex },
  });
  ...
  btnEasy.button.on("click", () => {
    try {
      localStorage.setItem("ash:difficulty", "Easy");
    } catch (e) {}
    app.fire("hud:update", { lit: 0, total: 0, difficulty: "Easy" });
  });
  btnNormal.button.on("click", () => {
    try {
      localStorage.setItem("ash:difficulty", "Normal");
    } catch (e) {}
    app.fire("hud:update", { lit: 0, total: 0, difficulty: "Normal" });
  });
  btnHard.button.on("click", () => {
    try {
      localStorage.setItem("ash:difficulty", "Hard");
    } catch (e) {}
    app.fire("hud:update", { lit: 0, total: 0, difficulty: "Hard" });
  });
  btnStart.button.on("click", () => {
    app.fire("game:reset");
    app.fire("game:resume");
    menuPanel.enabled = false;
    hudPanel.enabled = true;
  });
  btnResume.button.on("click", () => {
    app.fire("game:resume");
    pausePanel.enabled = false;
    hudPanel.enabled = true;
  });
  btnRestart.button.on("click", () => {
    app.fire("game:restart");
    pausePanel.enabled = false;
    hudPanel.enabled = true;
  });
  btnReturnToMain.button.on("click", () => {
    app.fire("game:pause");
